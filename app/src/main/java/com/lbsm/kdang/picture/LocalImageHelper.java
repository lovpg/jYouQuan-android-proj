...
public class LocalImageHelper {
    ...
    public Map<String,List<LocalFile>> foldres = new HashMap<String, List<LocalFile>>();
        ...
        if(cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                if (file.exists()){
                    ...
                    paths.add(localFile);
                    if (foldres.containsKey(folder)){
                        foldres.get(folder).add(localFile);
                    }else{
                        List<LocalFile> files = new ArrayList<LocalFile>();
                        files.add(localFile);
                        foldres.put(folder,files);
                    }
                }
            }
        }

        foldres.put("All picturres",paths);
        cursor.close();
    ...
    public List<LocalFile> getLocalFiles(String folder){
        return foldres.get(folder);
    }

    public void initFlodes(){
        Iterator iter = foldres.entrySet().iterator();
        while (iter.hasNext()) {
    ...
    public void isLocalFiles(final String folder){
        new Thread(){
            ...
            public void run() {
                ...
                for (LocalFile localFile : foldres.get(folder)){
