...
import com.lbsm.kdang.net.request.PostEssenceReq;
...
public class PostEssenceFragm extends BaseFragment {

    private static final String TAG = "PostEssenceFragm";
    ...
    private void initView() {
        if(mPostAdapter == null){
            if (mPosts != null && mPosts.size() > 0) {
                mPostAdapter = new PostAdapter(mContext, mPullListView, new PostEssenceReq(), false);
                mPostAdapter.addAll(mPosts);
            } else
                mPostAdapter = new PostAdapter(getActivity(), mPullListView, new PostEssenceReq(), true);
        }
    }
