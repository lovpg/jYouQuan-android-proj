...
import com.lbsm.kdang.net.request.PostNewReq;
...
public class PostNewFragm extends BaseFragment {

    private static final String TAG = "PostNewFragm";
    ...
    private void initView() {
        if(mPostAdapter == null){
            if (mPosts != null && mPosts.size() > 0) {
                mPostAdapter = new PostAdapter(mContext, mPullListView, new PostNewReq(), false);
                mPostAdapter.addAll(mPosts);
            } else
                mPostAdapter = new PostAdapter(getActivity(), mPullListView, new PostNewReq(), true);
        }
    }
